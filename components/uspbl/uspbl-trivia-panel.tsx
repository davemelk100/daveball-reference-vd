...
import {
  getDailyUSPBLTriviaQuestions,
  getUSPBLTodayStorageKey,
  type USPBLTriviaQuestion,
} from "@/lib/uspbl-trivia-data";
import {
  CheckCircle2,
  XCircle,
  ChevronLeft,
...
function USPBLTriviaPanelContent() {
  const [quizDate, setQuizDate] = useState<Date | null>(null);
  const [dailyQuestions, setDailyQuestions] = useState<USPBLTriviaQuestion[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [answeredQuestions, setAnsweredQuestions] = useState<AnsweredQuestion[]>([]);
  const [isComplete, setIsComplete] = useState(false);
  const [showYesterday, setShowYesterday] = useState(false);
  const [yesterdayQuestions, setYesterdayQuestions] = useState<USPBLTriviaQuestion[]>([]);

  useEffect(() => {
    const now = new Date();
    setQuizDate(now);

    const questions = getDailyUSPBLTriviaQuestions(now);
    setDailyQuestions(questions);

    const yesterday = new Date(now);
    yesterday.setDate(yesterday.getDate() - 1);
    setYesterdayQuestions(getDailyUSPBLTriviaQuestions(yesterday));

    const storageKey = getUSPBLTodayStorageKey(now);
  ...
  const handleAnswer = (index: number) => {
    ...
    const storageKey = getUSPBLTodayStorageKey(quizDate!);
  ...
  const handleShare = async () => {
    ...
    const text = `I got ${score}/5 on today's (${date}) USPBL Numbers trivia!\n\nPlay here: https://majorleaguenumbers.com/uspbl`;

    if (navigator.share) {
      try {
        await navigator.share({ title: "USPBL Numbers Trivia", text });
      } catch (err: any) {
