...
import {
  getDailyNFLTriviaQuestions,
  getNFLTodayStorageKey,
  type NFLTriviaQuestion,
} from "@/lib/nfl-trivia-data";
import {
  CheckCircle2,
  XCircle,
  ChevronLeft,
...
function NFLTriviaPanelContent() {
  const [quizDate, setQuizDate] = useState<Date | null>(null);
  const [dailyQuestions, setDailyQuestions] = useState<NFLTriviaQuestion[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [answeredQuestions, setAnsweredQuestions] = useState<AnsweredQuestion[]>([]);
  const [isComplete, setIsComplete] = useState(false);
  const [showYesterday, setShowYesterday] = useState(false);
  const [yesterdayQuestions, setYesterdayQuestions] = useState<NFLTriviaQuestion[]>([]);

  useEffect(() => {
    const now = new Date();
    setQuizDate(now);

    const questions = getDailyNFLTriviaQuestions(now);
    setDailyQuestions(questions);

    const yesterday = new Date(now);
    yesterday.setDate(yesterday.getDate() - 1);
    setYesterdayQuestions(getDailyNFLTriviaQuestions(yesterday));

    const storageKey = getNFLTodayStorageKey(now);
  ...
  const handleAnswer = (index: number) => {
    ...
    const storageKey = getNFLTodayStorageKey(quizDate!);
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: "NFL Numbers Trivia", text });
      } catch (err: any) {
